...
class NewSearchViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
...
//    }()
    ...
    @objc func presentSearchController(){
        print("TRIGGGERRREEEDDD")
        searchController.isActive = true
        
        DispatchQueue.main.async {
            print("FIRST RESPONDDEERR")
            self.searchController.searchBar.becomeFirstResponder()
        }
                ...
                for data in snapshot.children.allObjects as! [DataSnapshot] {
                    ...
                    if let dat = data.value as? [String: Any] {
                        ...
                        let userToAdd = userSearchCell()
                          ...
                          userToAdd.userData = PersonInfo(username: userName, uid: uid, bio: bio, fullName: fullName)
                    ...
                    if(userId != self.uid){
                     ref.child("users/\(userId)").observeSingleEvent(of: .value, with: { (snapshot) in
                      if let dat = snapshot.value as? [String: Any] {
                          ...
                          userToAdd.userData = PersonInfo(username: userName, uid: userID, bio: bio, fullName: fullName)
                          self.users?.append(userToAdd)
...
extension NewSearchViewController: UISearchResultsUpdating {
